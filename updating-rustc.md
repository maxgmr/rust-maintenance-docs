...
```shell
git push <lpname> merge-<X.Y>
```

Delete the previous `experimental` branch:
...
Consulting your `git log`, you should see the two new `gbp` commits immediately after the final commit of the last Rust version. Now, you can take a look at all your commits from your remote backup and cherry-pick them all (except the previous `gbp` commits) on _top_ of your new upstream source:

```shell
# Cherry-pick all the commits from here (except your previous gbp import, of course)
git log <remote>/merge-<X.Y>
```

#### Verifying your Git changes

Before you force-push to your personal Launchpad remote, you _really_ want to make sure you didn't miss a single thing. To do that, compare your branch with your remote "backup":

```shell
git diff <remote>/merge-<X.Y> --name-status
```

You should see ONLY a huge list of deleted `vendor/` files. If there are _any other changes_, then you forgot to cherry-pick something!

Once you're 100% sure everything is good, force push to your remote "backup":
